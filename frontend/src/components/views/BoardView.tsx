...
import { Plus, Calendar, KanbanSquare as LayoutKanban } from 'lucide-react' // GripVertical silindi
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
// YENİ: Tipleri ayırdık ("import type" kullanarak)
...
import {
  ...
  useSortable,
} from '@dnd-kit/sortable' // arrayMove silindi
...
// 'id' prop'u kullanılmadığı için listeden çıkarıldı
const SortableItem = ({ page, properties, pageValues, groupProperty, onOpenPage }: any) => {
    const {
        ...
        transition,
        isDragging
    } = useSortable({ id: page.id, data: { page } })
...
// --- ANA PANO BİLEŞENİ ---
// 'databaseId' kullanılmadığı için çıkarıldı
export default function BoardView({ properties, pages, pageValues, onAddPage, onOpenPage, onOpenStatusModal, onUpdateValue }: BoardViewProps) {
    ...
    // Sensörler (Sürükleme hassasiyeti)
    ...
    // Sütunları ve İçeriklerini Hazırla (Memoized)
    ...
    if (!groupProperty) {
        return (
            <div className="flex flex-col items-center justify-center h-64 border border-dashed border-[#373737] rounded-lg text-gray-500 gap-3 m-4">
                <LayoutKanban size={48} className="opacity-20"/>
                <p>Pano görünümü için veritabanında en az bir <strong>Durum (Status)</strong> veya <strong>Seçim (Select)</strong> özelliği olmalıdır.</p>
            </div>
        )
    }
    ...
    const handleDragOver = () => {
        // Boş bırakıldı, event parametresi silindi
    }
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (columns[overId]) {
            targetContainerId = overId
             ...
             for (const [colId, items] of Object.entries(columns)) {
                 if (items.find(p => p.id === overId)) {
                     targetContainerId = colId
                     break
                 }
             }
        }
        ...
        if (targetContainerId) {
             ...
             // Gereksiz 'targetCheck' değişkeni silindi
    ...
    return (
        ...
        >
            <div className="flex gap-4 overflow-x-auto p-4 h-[calc(100vh-200px)] items-start bg-[#191919]">
                ...
                {columns['uncategorized'] && columns['uncategorized'].length > 0 && (
                    <BoardColumn 
                        ...
                        count={columns['uncategorized'].length}
                        color="default"
                        onAddPage={() => onAddPage()}
                ...
                {options.map((opt: any) => (
                    <BoardColumn 
                        ...
                        count={(columns[opt.id] || []).length}
                        color={opt.color}
                        onAddPage={() => onAddPage(opt.id)}
                        onOpenPage={onOpenPage}
                        properties={properties}
                        pageValues={pageValues}
                        groupProperty={groupProperty}
                ...
                <div className="min-w-[120px] pt-2 shrink-0">
                    <button 
                        onClick={() => onOpenStatusModal(groupProperty.id, groupProperty.name, options, groupProperty.type)}
                        className="flex items-center justify-center gap-2 text-gray-400 hover:text-white text-sm px-4 hover:bg-[#2C2C2C] py-3 rounded-xl border border-dashed border-[#373737] transition-all font-medium w-full h-[60px]"
                    >
            ...
            <DragOverlay dropAnimation={{ sideEffects: defaultDropAnimationSideEffects({ styles: { active: { opacity: '0.5' } } }) }}>
                {activeId ? (
                   <div className="bg-[#2C2C2C] p-3 rounded-lg shadow-2xl border border-blue-500 cursor-grabbing rotate-2">
                       <div className="w-40 h-8 bg-gray-600/20 rounded animate-pulse"></div>
                   </div>
                ) : null}
            </DragOverlay>
        </DndContext>
    ...
    return (
        <div ref={setNodeRef} style={colStyle} className={`min-w-[280px] w-[280px] flex flex-col gap-3 shrink-0 p-2 rounded-xl transition-all max-h-full ${colClass}`}>
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pb-2 custom-scrollbar pr-1 min-h-[50px]">
                ...
                <button className="flex items-center gap-2 text-gray-400 hover:text-white text-sm p-2 rounded hover:bg-black/10 w-full transition-colors mt-1 font-medium" onClick={onAddPage}>
                    <Plus size={16}/> Yeni
                </button>
            </div>
        </div>
